...
class DSplitter:
    ...
    """
    def __init__(self, vpaths, groups, ldict, split_info_file):
        """ 
        ...
        groups: list of str
            List of groups that are considered to for data splitting. If empty
            all groups are considered.
        ldict: Dict
        ...
        self._tvdf = self._create_tv_df(vpaths, ldict, groups)

        # Shuffle
        self._tvdf.sample(frac=1).reset_index(drop=True)

    def _create_tv_df(self, paths, ldict, groups):
        """ Returns a data frame with following columns
        1. `activity`, 2. `vname`, 3. `num_label`, 4. `split`, 5. `splabel`, 6. `glabel`

        Note
        ----
        + `splabel`means session-person-label.
        + `glabel` means group label
        ...
        # group label, ex: C1L1P-A
        glabel = [self._get_group_label(x) for x in vnames]

        # Zipping
        tvlist_of_tuples = list(
            zip(vlabels, vnames, vnlabels, split_label, splabel, glabel))

        # Converting each tuple to list
        tvlist = [list(a) for a in tvlist_of_tuples]

        # Convert to dataframe
        columns = ['activity', 'vname', 'num_label', 'split', 'splabel', 'glabel']
        df = pd.DataFrame(tvlist, columns=columns)

        # Dataframe that have valid groups.
        if groups:
            df2 = self._get_df_with_valid_groups(df, groups)
            return df2
        else:
            return df
    ...
    def _get_df_with_valid_groups(self, df, groups):
        """ Returns a data frame that has only valid groups
        df: DataFrame
            Data frame having information of all groups
        groups: list of str
            List of groups that are considered to for data splitting.
        """
        for i, group in enumerate(groups):
            df_tmp = df[df['glabel']==group].copy()
            if i == 0:
                df2 = df_tmp
            else:
    ...
    def _get_session_and_person_label(self, vname):
        ...
        from video name. For example, C1L1P-Feb27-A-Kid11.
        tv_0_G-C2L1W-Feb27-B-Issac_q2_01-04_30fps_4828_4918_Kidx_rzto_224.mp4

        Todo
        ----
        - Support for multiple cameras is not available.
        """
    ...
    def _get_group_label(self, vname):
        """ Returns group label. For example, C1L1P-A.
        tv_0_G-C2L1W-Feb27-B-Issac_q2_01-04_30fps_4828_4918_Kidx_rzto_224.mp4
    ...
    def _create_subsampled_video_list_files(self, odir, num_samples):
        ...
        for act in activities:
            ...
            self._create_subsampled_txt_list(trn_f, dftrn, num_trn_samples)

            # Creating list for validation samples
            self._create_subsampled_txt_list(val_f, dfval, num_val_samples)

            # Creating list for testing samples
            self._create_subsampled_txt_list(tst_f, dftst, num_tst_samples)

        trn_f.close()
        val_f.close()
        tst_f.close()

    def _create_subsampled_txt_list(self, f, dflst, n):
        """ Writes trimmed video samples into the file
        ...
        while (extracted_samples < n):
            ...
            for df_idx, df in enumerate(dflst):
                ...
                if len(df) > 0:
                    ...
                    # Randomly pop a sample from dataframe
    ...
    def _create_txt_list(self, f, df):
        """
        Create text file with entries in the data frame.
        ...
        f: File pointer
            ...
            Ex. `open("temp.txt", "w")`
        df: DataFrames
            DataFrame having entries to write into file
            handle f
        """
        for ridx, row in df.iterrows():
            line = (f"{row['activity']}/{row['vname']} "
                    f"{row['num_label']}\n")
            f.write(line)
        


    def _create_group_leave_one_out(self, odir):
        """ Creates group based leave one out training and validation
        text files.
        """
        trn_df = self._tvdf[self._tvdf['split'] == "trn"].copy()
        val_df = self._tvdf[self._tvdf['split'] == "val"].copy()
        trn_val_df = pd.concat([trn_df, val_df])

        # Unique group list
        uniq_grps = trn_val_df['glabel'].unique().tolist()

        for cgrp in uniq_grps:

            cgrp_odir = f"{odir}/{cgrp}"
            os.makedirs(cgrp_odir)
            print(f"INFO: trn.txt and val.txt create for {cgrp}")

            # Training and validation dataframes
            dfval = trn_val_df[trn_val_df['glabel']==cgrp]
            dftrn = trn_val_df[trn_val_df['glabel']!=cgrp]

            # Write training list
            trnf = open(f"{cgrp_odir}/trn.txt","w")
            self._create_txt_list(trnf, dftrn)
            trnf.close()

            # Write validation list
            valf = open(f"{cgrp_odir}/val.txt","w")
            self._create_txt_list(valf, dfval)
            valf.close()
