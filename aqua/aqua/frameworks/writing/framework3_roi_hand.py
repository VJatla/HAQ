...
class Writing3:
    ...
    def extract_writing_proposals_using_roi(self, overwrite = True, model_fps = None):
        """Extracts writing region proposals using ROI to a directory (`odir`).

        It write the output to `wrp_only_roi.csv` adding an extra
        ...
        overwrite : Bool, optional
            Overwrite existing writing proposals. Defaults to True.
        ...
        for i, video_name in enumerate(video_names):
            ...
            print(f"Extracting writing region proposals from: {video_name}")
            ...
            for ii, row in tqdm(wrp_video.iterrows(), total=wrp_video.shape[0], desc="Extracting: "):
        ...
        if "proposal_name" in self.wrp_roi_only.columns:
            self.wrp_roi_only.drop("proposal_name", inplace=True, axis=1)
            self.wrp_roi_only['proposal_name'] = prop_name_lst
        else:
            self.wrp_roi_only['proposal_name'] = prop_name_lst
        wrp_roi_only_loc = f"{self.cfg['oloc']}/wrp_only_roi.csv"
        print(f"Rewriting {wrp_roi_only_loc}")
    ...
    def extract_writing_proposals_using_roi_kbdet(self, overwrite = True, model_fps = None):
        """Extracts writing region proposals using ROI to a directory (`odir`).
    ...
    def classify_writing_proposals_roi_fast_approach(self, overwrite=False, batch_size = 4, kb_det=False):
        """Classify each proposed region as writing / no-writing. This method needs the
        ...
        if kb_det:
            out_file = f"{self.cfg['oloc']}/wnw-roi-ours-{3*self.cfg['model_fps']}-kbdet.csv"
        else:
            out_file = f"{self.cfg['oloc']}/wnw-roi-ours-{3*self.cfg['model_fps']}.csv"

        # Creating default columns for activity, class_idx and class_prob
        self.wrp_roi_only["activity"] = "nowriting"
        self.wrp_roi_only["class_idx"] = 0
        self.wrp_roi_only["class_prob"] = 0
        ...
        # creating wdf by copying self.wrp_only_roi
        self.wdf = self.wrp_roi_only.copy()
        ...
        for i, proposal_name in enumerate(proposal_names):
            ...
            loc = self.wdf[self.wdf['proposal_name']==proposal_name].index.tolist()
            if len(loc) > 1:
                print(f"Multiple rows having same proposal name! {loc}")
                import pdb; pdb.set_trace()

            self.wdf.loc[loc[0], "activity"] = pred_class
            self.wdf.loc[loc[0], "class_idx"] = pred_class_idx
            self.wdf.loc[loc[0], "class_prob"] = pred_class_prob


        # Saving the csv file
        print(f"INFO: Saving {out_file}")
        self.wdf.to_csv(out_file, index=False) 
