...
def write_to_video(vpath, df):
    """
    Writes a video with typing/notyping labeled. The video
    has name <video name>_ty_using_alg.avi
    ...
    for i, row in df.iterrows():
        for poc in range(int(row['f0'].item()), int(row['f0'].item() + row['f'].item()), 10):
            ...
            if row['typing'] == -1:
                frm = cv2.putText(frm,'Keyboard not detected', 
                ...
                raise Exception(f"{row['typing']} label not supported")
            out.write(frm)
...
def _arguments():
    ...
    args_inst.add_argument("tycsv", type=str, help=("<pos arg help>"))
    # Parse arguments
