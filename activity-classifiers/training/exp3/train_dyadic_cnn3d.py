...
def _arguments():
    ...
    args_inst.add_argument("seed", type=str, help=("Random seed"))
...
def main():
    ...
    # Setting seed here too just in case the weights get initialized in
    ...
    # Build model
    cnn3d = DyadicCNN3D(ndyads, [3, 90, 224, 224], seed=seed)

    # Loss and optimizer
    criterion = nn.BCELoss()  # Using Binary Cross Entropy loss
    optimizer = optim.SGD(cnn3d.parameters(),
                          lr=0.001,
                          momentum=0.9,
                          nesterov=True)
